...
class MeetingMode:
    ...
    def start(self):
        ...
        # Create DB record (after recorder.start so has_system_audio is set)
        self._meeting_id = db.start_meeting(
            has_system_audio=self.recorder.has_system_audio
        )
    ...
    def stop(self) -> str:
        ...
        if self._meeting_id is not None:
            ...
            db.end_meeting(self._meeting_id, duration)
    ...
    def _process(self, audio: np.ndarray):
        ...
        for start, end, text in segs:
            ...
            # Write to DB
            if self._meeting_id is not None:
                db.add_segment(self._meeting_id, start, end, text)
    ...
    def _write_header(self):
        if self._session_file:
            with open(self._session_file, "w", encoding="utf-8") as f:
                f.write("freewispr Meeting Transcript\n")
